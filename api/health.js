...
var STANDALONE_KEYS = {
  ...
  displacement: `displacement:summary:v1:${(/* @__PURE__ */ new Date()).getFullYear()}`,
...
var SEED_META = {
  ...
  // runs as independent Railway cron (0 */2 * * *)
  ...
  // 2h cron; 3x interval; was 120 (TTL was 60min — panel went dark before health alarmed)
  ...
  // CII warm-ping every 8min; 30min = ~3.5x interval,
  ...
  // 6h cron + 1h grace; CACHE_TTL is 24h so per-topic merge always has a prior snapshot
  ...
  // same key as flightDelays; CACHE_TTL=7200s
  ...
  consumerPricesOverview: { key: "seed-meta:consumer-prices:overview:ae", maxStaleMin: 90 },
  // seed TTL=30min; 3× interval
  ...
  consumerPricesSpread: { key: "seed-meta:consumer-prices:retailer-spread:ae:essentials-ae", maxStaleMin: 120 },
  // TTL=60min; 2× interval
  consumerPricesFreshness: { key: "seed-meta:consumer-prices:freshness:ae", maxStaleMin: 90 },
  // publish.ts runs independently; align with other consumer-prices keys
