// api/tree-canopy.js
// Edge Function: serves tree canopy data from ArcGIS
// Cache: 90-day TTL
...
var DATA_URL = "https://gis.toronto.ca/arcgis/rest/services/cot_geospatial13/FeatureServer/43/query?f=geojson&outSR=4326&where=1%3D1&outFields=*&resultRecordCount=50";
var CACHE_KEY = "tree-canopy";
var CACHE_TTL_SECONDS = 90 * 24 * 60 * 60; // 90 days
...
async function handler(req) {
  ...
  try {
    ...
    var res = await fetch(DATA_URL, {
      signal: AbortSignal.timeout(15000), // 15 second timeout for large ArcGIS responses
    });
    if (!res.ok) {
    ...
    for (var _i = 0; _i < features.length; _i++) {
      ...
      areas.push({
        id: props.OBJECTID || props.id || ("canopy-" + _i),
    ...
    console.error("Tree Canopy API error:", err);
    return jsonResponse({
      areas: [],
      total: 0,
      lastUpdated: now,
