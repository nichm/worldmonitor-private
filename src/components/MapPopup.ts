...
export class MapPopup {
  ...
  public show(data: PopupData): void {
    ...
    this.popup.className = this.isMobileSheet
    ...
    this.popup.addEventListener("click", (e) => {
      ...
      if (
        ...
        target.closest(".map-popup-sheet-handle")
      ) {
        this.hide();
        return;
      }
    ...
    if (this.isMobileSheet) {
      this.popup.addEventListener("touchstart", this.handleSheetTouchStart, {
        passive: true,
      });
      this.popup.addEventListener("touchmove", this.handleSheetTouchMove, {
        passive: false,
      });
  ...
  private handleSheetTouchEnd = (): void => {
    ...
    const shouldDismiss =
      this.sheetCurrentOffset >= this.mobileDismissThreshold;
  ...
  private renderContent(data: PopupData): string {
    switch (data.type) {
      ...
      case "datacenterCluster":
        return this.renderDatacenterClusterPopup(
          data.data as DatacenterClusterData,
        );
      case "ais":
      ...
      case "militaryFlightCluster":
        return this.renderMilitaryFlightClusterPopup(
          data.data as MilitaryFlightCluster,
        );
      case "militaryVesselCluster":
        return this.renderMilitaryVesselClusterPopup(
          data.data as MilitaryVesselCluster,
        );
      case "natEvent":
      ...
      case "techEventCluster":
        return this.renderTechEventClusterPopup(
          data.data as TechEventClusterData,
        );
      case "stockExchange":
        return this.renderStockExchangePopup(
          data.data as StockExchangePopupData,
        );
      case "financialCenter":
        return this.renderFinancialCenterPopup(
          data.data as FinancialCenterPopupData,
        );
      case "centralBank":
  ...
  private renderHotspotPopup(
    ...
    const displayScore =
      dynamicScore?.combinedScore ?? hotspot.escalationScore ?? 3;
    const displayScoreInt = Math.round(displayScore);
    const displayTrend =
      dynamicScore?.trend ?? hotspot.escalationTrend ?? "stable";
    ...
    const escalationSection = `
      <div class="popup-section escalation-section">
        ...
        <div class="escalation-display">
          ...
          ${
            dynamicScore?.history && dynamicScore.history.length >= 3
              ? (() => {
                  const vals = dynamicScore.history
                    .slice(-20)
                    .map((h) => h.score);
        ...
        ${
          hotspot.escalationIndicators &&
          hotspot.escalationIndicators.length > 0
            ? `
  ...
  private getTimeUntil(date: Date | string): string {
    ...
    if (hours < 1)
      return `${Math.floor(ms / (1000 * 60))}${t("popups.timeUnits.m")}`;
  ...
  private renderAisPopup(event: AisDisruptionEvent): string {
    ...
    const typeLabel =
      event.type === "gap_spike"
        ? t("popups.aisGapSpike")
        : t("popups.chokepointCongestion");
    const changeLabel =
      event.type === "gap_spike" ? t("popups.darkening") : t("popups.density");
    const countLabel =
      event.type === "gap_spike"
        ? t("popups.darkShips")
        : t("popups.vesselCount");
    const countValue =
      event.type === "gap_spike"
  ...
  private renderProtestPopup(event: SocialUnrestEvent): string {
    const severityClass = escapeHtml(event.severity);
    const severityLabel = escapeHtml(event.severity.toUpperCase());
    const eventTypeLabel = escapeHtml(
      event.eventType.replace("_", " ").toUpperCase(),
    );
    const icon =
      event.eventType === "riot"
        ? "🔥"
        : event.eventType === "strike"
          ? "✊"
          : "📢";
    const sourceLabel =
      event.sourceType === "acled"
        ? t("popups.protest.acledVerified")
        : t("popups.protest.gdelt");
  ...
  private renderProtestClusterPopup(data: ProtestClusterData): string {
    ...
    const riots =
      data.riotCount ?? data.items.filter((e) => e.eventType === "riot").length;
    const highSeverity =
      data.highSeverityCount ??
      data.items.filter((e) => e.severity === "high").length;
    const verified =
      data.verifiedCount ?? data.items.filter((e) => e.validated).length;
    const totalFatalities =
      data.totalFatalities ??
      data.items.reduce((sum, e) => sum + (e.fatalities || 0), 0);
    ...
    const moreCount =
      remainingCount > 0
        ...
        : "";
    const headerClass =
      highSeverity > 0 ? "high" : riots > 0 ? "medium" : "low";
  ...
  private renderAircraftPopup(pos: PositionSample): string {
    ...
    const statusLabel = pos.onGround
      ? t("popups.aircraft.ground")
      : t("popups.aircraft.airborne");
    const altDisplay =
      pos.altitudeFt > 0
...
${isFeatureAvailable("wingbitsEnrichment") ? '<div class="wingbits-live-section"><div class="wingbits-live-loading" style="font-size:11px;opacity:0.5;padding:4px 0">Loading Wingbits live data…</div></div>' : ""}
  ...
  private renderEconomicPopup(center: EconomicCenter): string {
    ...
    const marketStatus = center.marketHours
      ? this.getMarketStatus(center.marketHours)
      : null;
  ...
  private renderPipelinePopup(pipeline: Pipeline): string {
    ...
    const typeIcon =
      pipeline.type === "oil" ? "🛢" : pipeline.type === "gas" ? "🔥" : "⛽";
  ...
  private renderCablePopup(cable: UnderseaCable): string {
    ...
    if (healthRecord?.status === "fault") {
      ...
      statusLabel =
        advisory.severity === "fault"
          ? t("popups.cable.fault")
          : t("popups.cable.degraded");
    ...
    const advisoryDescription = advisory
  ...
  private renderCableAdvisoryPopup(advisory: CableAdvisory): string {
    ...
    const statusLabel =
      advisory.severity === "fault"
        ? t("popups.cable.fault")
        : t("popups.cable.degraded");
    const cableName = escapeHtml(
      cable?.name.toUpperCase() || advisory.cableId.toUpperCase(),
    );
    const advisoryTitle = escapeHtml(advisory.title);
    const advisoryImpact = escapeHtml(advisory.impact);
    const advisoryEta = advisory.repairEta
      ? escapeHtml(advisory.repairEta)
      : "";
  ...
  private renderRepairShipPopup(ship: RepairShip): string {
    ...
    const shipNote = escapeHtml(
      ship.note || t("popups.repairShip.description"),
    );
  ...
  private getLatestCableAdvisory(cableId: string): CableAdvisory | undefined {
    const advisories = this.cableAdvisories.filter(
      (item) => item.cableId === cableId,
    );
    return advisories.reduce<CableAdvisory | undefined>((latest, advisory) => {
      if (!latest) return advisory;
      return advisory.reported.getTime() > latest.reported.getTime()
        ? advisory
        : latest;
    }, undefined);
  }
  ...
  private renderDatacenterClusterPopup(data: DatacenterClusterData): string {
    const totalCount = data.count ?? data.items.length;
    const totalChips =
      data.totalChips ?? data.items.reduce((sum, dc) => sum + dc.chipCount, 0);
    const totalPower =
      data.totalPowerMW ??
      data.items.reduce((sum, dc) => sum + (dc.powerMW || 0), 0);
    const existingCount =
      data.existingCount ??
      data.items.filter((dc) => dc.status === "existing").length;
    const plannedCount =
      data.plannedCount ??
      data.items.filter((dc) => dc.status === "planned").length;
  ...
  private renderTechEventPopup(event: TechEventPopupData): string {
    ...
    const urgencyClass =
      event.daysUntil <= 7 ? "urgent" : event.daysUntil <= 30 ? "soon" : "";
    const daysLabel =
      event.daysUntil === 0
        ? t("popups.techEvent.days.today")
        : event.daysUntil === 1
          ? t("popups.techEvent.days.tomorrow")
          : t("popups.techEvent.days.inDays", {
              count: String(event.daysUntil),
            });
  ...
  private renderTechHQClusterPopup(data: TechHQClusterData): string {
    ...
    const unicornCount =
      data.unicornCount ??
      data.items.filter((h) => h.type === "unicorn").length;
    const faangCount =
      data.faangCount ?? data.items.filter((h) => h.type === "faang").length;
    const publicCount =
      data.publicCount ?? data.items.filter((h) => h.type === "public").length;
    ...
    const listItems = sortedItems
      .map((hq) => {
        const icon =
          hq.type === "faang" ? "🏛️" : hq.type === "unicorn" ? "🦄" : "🏢";
  ...
  private renderTechEventClusterPopup(data: TechEventClusterData): string {
    const totalCount = data.count ?? data.items.length;
    const upcomingSoon =
      data.soonCount ?? data.items.filter((e) => e.daysUntil <= 14).length;
...
${isFeatureAvailable("wingbitsEnrichment") ? '<div class="wingbits-live-section"><div class="wingbits-live-loading" style="font-size:11px;opacity:0.5;padding:4px 0">Loading Wingbits live data…</div></div>' : ""}
  ...
  private getOperatorCountryCode(vessel: {
    operator: string;
    operatorCountry?: string;
  }): string {
    return (
      (vessel.operatorCountry
        ? nameToCountryCode(vessel.operatorCountry)
        : null) ||
  ...
  private renderMilitaryVesselPopup(vessel: MilitaryVessel): string {
    ...
    const vesselOperator = escapeHtml(
      operatorLabels[vessel.operator] ||
        vessel.operatorCountry ||
        t("popups.unknown"),
    );
    ...
    const usniIntel =
      vessel.usniActivityDescription ||
      vessel.usniRegion ||
  ...
  private renderMilitaryFlightClusterPopup(
    cluster: MilitaryFlightCluster,
  ): string {
  ...
  private renderMilitaryVesselClusterPopup(
    ...
    const dominantOp = Object.entries(opCounts).sort(
      (a, b) => b[1] - a[1],
    )[0]?.[0];
    const dominantCode = dominantOp
      ? MapPopup.OPERATOR_COUNTRY_MAP[dominantOp] || ""
      : "";
            ...
            ${event.relatedEvents
              .map((r) => {
                ...
                const rTime = r.timestamp
                  ? this.getTimeAgo(new Date(r.timestamp))
                  : "";
                const rTitle =
...
function formatRadiationConfidence(
