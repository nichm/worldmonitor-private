...
const BASES_ICON_MAPPING = {
  triangleUp: { x: 0, y: 0, width: 32, height: 32, mask: true },
};
const NUCLEAR_ICON_MAPPING = {
  hexagon: { x: 0, y: 0, width: 32, height: 32, mask: true },
};
const DATACENTER_ICON_MAPPING = {
  square: { x: 0, y: 0, width: 32, height: 32, mask: true },
};
...
const ALTITUDE_COLOR_STOPS: Array<{
  alt: number;
  r: number;
  g: number;
  b: number;
...
export class DeckGLMap {
  ...
  private speciesRecoveryZones: Array<
    SpeciesRecovery & {
      recoveryZone: { name: string; lat: number; lon: number };
    }
  > = [];
  ...
  // CII choropleth data
  private ciiScoresMap: Map<string, { score: number; level: string }> =
    new Map();
  ...
  constructor(container: HTMLElement, initialState: DeckMapState) {
    ...
    this.debouncedFetchAircraft = debounce(
    ...
    this.handleThemeChange = () => {
      ...
      const paintTheme = isLightMapTheme(mapTheme)
        ? ("light" as const)
        : ("dark" as const);
    ...
    if (
      this.state.layers.cyberThreats &&
      SITE_VARIANT !== "tech" &&
      SITE_VARIANT !== "happy"
    ) {
      this.loadAptGroups();
  ...
  private startWeatherRadar(): void {
    ...
    if (!this.radarRefreshIntervalId) {
      this.radarRefreshIntervalId = setInterval(
        () => this.fetchAndApplyRadar(),
        5 * 60 * 1000,
      );
    }
  ...
  private fetchAndApplyRadar(): void {
    fetch("https://api.rainviewer.com/public/weather-maps.json")
      .then((r) => r.json())
      .then(
        (data: { host: string; radar: { past: Array<{ path: string }> } }) => {
  ...
  private initMapLibre(): void {
    ...
    const initialProvider = isHappyVariant
    ...
    const recreateWithFallback = () => {
      ...
      const fallback = isLightMapTheme(initialMapTheme)
        ...
        : FALLBACK_DARK_STYLE;
      console.warn(
        `[DeckGLMap] Primary basemap failed, recreating with fallback: ${fallback}`,
      );
      const attr = this.container.querySelector(".map-attribution");
      if (attr)
        attr.innerHTML =
    ...
    canvas.addEventListener("webglcontextlost", (e) => {
      ...
      this.webglLost = true;
      console.warn(
        "[DeckGLMap] WebGL context lost — will restore when browser recovers",
      );
    });
  ...
  private initDeck(): void {
    ...
    this.maplibreMap.on("moveend", () => {
      ...
      if (this.state.layers.satellites) {
        ...
        this.imagerySearchTimer = setTimeout(
  ...
  private filterMilitaryFlightClustersByTime(
    ...
    return clusters
      .map((cluster) => {
        const flights = this.filterByTime(
          cluster.flights ?? [],
  ...
  private filterMilitaryVesselClustersByTime(
    ...
    return clusters
      .map((cluster) => {
        const vessels = this.filterByTime(
          cluster.vessels ?? [],
  ...
  private rebuildProtestSupercluster(
    ...
    this.protestSC = new Supercluster({
      ...
      map: (props: Record<string, unknown>) => ({
        ...
        maxSeverityRank:
          props.severity === "high" ? 2 : props.severity === "medium" ? 1 : 0,
      ...
      reduce: (
        ...
        acc.riotCount =
          Number(acc.riotCount ?? 0) + Number(props.riotCount ?? 0);
        acc.highSeverityCount =
          Number(acc.highSeverityCount ?? 0) +
          Number(props.highSeverityCount ?? 0);
        acc.verifiedCount =
          Number(acc.verifiedCount ?? 0) + Number(props.verifiedCount ?? 0);
        acc.totalFatalities =
          Number(acc.totalFatalities ?? 0) + Number(props.totalFatalities ?? 0);
        const accRiot = Number(acc.riotTimeMs ?? 0);
        const propRiot = Number(props.riotTimeMs ?? 0);
        acc.riotTimeMs = Number.isFinite(propRiot)
          ? Math.max(accRiot, propRiot)
          : accRiot;
  ...
  private rebuildTechHQSupercluster(): void {
    ...
    this.techHQSC = new Supercluster({
      ...
      reduce: (
        ...
        acc.faangCount =
          Number(acc.faangCount ?? 0) + Number(props.faangCount ?? 0);
        acc.unicornCount =
          Number(acc.unicornCount ?? 0) + Number(props.unicornCount ?? 0);
        acc.publicCount =
          Number(acc.publicCount ?? 0) + Number(props.publicCount ?? 0);
    ...
    this.techEventSC = new Supercluster({
      ...
      map: (props: Record<string, unknown>) => {
        ...
        return {
          ...
          country: String(props.country ?? ""),
          soonestDaysUntil: Number.isFinite(daysUntil)
            ? daysUntil
      ...
      reduce: (
        ...
        acc.soonCount =
          Number(acc.soonCount ?? 0) + Number(props.soonCount ?? 0);
  ...
  private rebuildDatacenterSupercluster(): void {
    const activeDCs = AI_DATA_CENTERS.filter(
      (dc) => dc.status !== "decommissioned",
    );
    ...
    const points = activeDCs.map((dc, i) => ({
      ...
      geometry: {
        type: "Point" as const,
        coordinates: [dc.lon, dc.lat] as [number, number],
      },
      properties: {
        index: i,
        country: dc.country,
    ...
    this.datacenterSC = new Supercluster({
      ...
      reduce: (
        ...
        acc.totalChips =
          Number(acc.totalChips ?? 0) + Number(props.totalChips ?? 0);
        acc.totalPowerMW =
          Number(acc.totalPowerMW ?? 0) + Number(props.totalPowerMW ?? 0);
        acc.existingCount =
          Number(acc.existingCount ?? 0) + Number(props.existingCount ?? 0);
        acc.plannedCount =
          Number(acc.plannedCount ?? 0) + Number(props.plannedCount ?? 0);
        if (!acc.country && props.country) acc.country = props.country;
      },
    });
  ...
  private updateClusterData(): void {
    ...
    const useProtests =
      layers.protests && this.protestSuperclusterSource.length > 0;
    const useTechHQ = SITE_VARIANT === "tech" && layers.techHQs;
    const useTechEvents =
      SITE_VARIANT === "tech" &&
      layers.techEvents &&
      this.techEvents.length > 0;
    ...
    if (useTechEvents && this.techEventSC) {
      this.techEventClusters = this.techEventSC
        ...
        .map((f) => {
          ...
          if (f.properties.cluster) {
            const props = f.properties as Record<string, unknown>;
            const clusterCount = Number(f.properties.point_count ?? 0);
            const soonestDaysUntil = Number(
              props.soonestDaysUntil ?? Number.MAX_SAFE_INTEGER,
            );
    ...
    if (useDatacenterClusters && this.datacenterSC) {
      ...
      this.datacenterClusters = this.datacenterSC
        .getClusters(bbox, zoom)
        .map((f) => {
          const coords = f.geometry.coordinates as [number, number];
          if (f.properties.cluster) {
  ...
  private buildLayers(): LayersList {
    ...
    const filteredCableAdvisories = mapLayers.cables
      ? this.filterByTimeCached(
          this.cableAdvisories,
          (advisory) => advisory.reported,
        )
      : [];
    ...
    const filteredMilitaryFlightClusters = mapLayers.military
      ? this.filterMilitaryFlightClustersByTimeCached(
          this.militaryFlightClusters,
        )
      : [];
    const filteredMilitaryVesselClusters = mapLayers.military
      ? this.filterMilitaryVesselClustersByTimeCached(
    ...
    if (mapLayers.flights && filteredFlightDelays.length > 0) {
      ...
      const closures = filteredFlightDelays.filter(
        (d) => d.delayType === "closure",
      );
    ...
    if (mapLayers.military && filteredMilitaryVesselClusters.length > 0) {
      layers.push(
        this.createMilitaryVesselClustersLayer(filteredMilitaryVesselClusters),
      );
    }
    ...
    if (mapLayers.military && filteredMilitaryFlightClusters.length > 0) {
      layers.push(
        this.createMilitaryFlightClustersLayer(filteredMilitaryFlightClusters),
      );
    }
    ...
    if (
      mapLayers.renewableInstallations &&
      this.renewableInstallations.length > 0
    ) {
    ...
    if (
      mapLayers.toronto_fire_incidents &&
      this.torontoFireIncidents.length > 0
    ) {
      layers.push(
        new ScatterplotLayer<TorontoFireIncident>({
          id: "toronto-fire-layer",
          data: this.torontoFireIncidents.filter(
            (inc) => inc.lat !== null && inc.lon !== null,
          ),
    ...
    // Ontario Highway Incidents
    if (mapLayers.ontario_roads && this.ontarioRoadsIncidents.length > 0) {
      layers.push(
        new ScatterplotLayer<OntarioRoadIncident>({
          id: "ontario-roads-layer",
          data: this.ontarioRoadsIncidents.filter(
          ...
          getFillColor: (d) => {
            switch (d.severity) {
              case "CRITICAL":
                return [220, 38, 38, 220]; // red
              case "MAJOR":
                return [249, 115, 22, 220]; // orange
              case "MODERATE":
                return [234, 179, 8, 220]; // yellow
              case "MINOR":
      ...
      layers.push(
        new GeoJsonLayer<any, any>({
          ...
          getFillColor: (d: any) => {
            ...
            const g = Math.min(
              255,
              Math.floor(((100 - riskScore) / 100) * 255),
            );
            return [r, g, 0, 80] as [number, number, number, number];
          },
          getLineColor: [200, 200, 200, 100] as [
            number,
            number,
            number,
            number,
    ...
    if (mapLayers.webcams && this.webcamData.length > 0) {
      layers.push(
        new ScatterplotLayer<WebcamEntry | WebcamCluster>({
          ...
          getRadius: (d) =>
            "count" in d ? Math.min(8 + d.count * 0.5, 24) : 6,
          getFillColor: (d) =>
  ...
  private buildConflictZoneGeoJson(): GeoJSON.FeatureCollection {
    ...
    for (const zone of CONFLICT_ZONES) {
      ...
      features.push({
        ...
        geometry: {
          type: "Polygon",
          coordinates: [ensureClosedRing(zone.coords)],
        },
      });
    }
  ...
  private createFlightDelaysLayer(
    delays: AirportDelayAlert[],
  ): ScatterplotLayer {
    return new ScatterplotLayer({
      ...
      getFillColor: (d) => {
        if (d.severity === "severe")
          return [255, 50, 50, 200] as [number, number, number, number];
        if (d.severity === "major")
          return [255, 150, 0, 200] as [number, number, number, number];
  ...
  private createNotamOverlayLayer(
    closures: AirportDelayAlert[],
  ): ScatterplotLayer {
    return new ScatterplotLayer({
  ...
  private createAircraftPositionsLayer(): IconLayer<PositionSample> {
    return new IconLayer<PositionSample>({
      ...
      getColor: (d) => {
        if (d.onGround)
          return [120, 120, 120, 160] as [number, number, number, number];
  ...
  private static readonly TC_WIND_COLORS: [
    number,
    [number, number, number, number],
  ][] = [
  ...
  private createGpsJammingLayer(): H3HexagonLayer {
    return new H3HexagonLayer({
      ...
      getFillColor: (d: GpsJamHex) => {
        if (d.level === "high")
          return [255, 80, 80, 180] as [number, number, number, number];
  ...
  private createCableAdvisoriesLayer(
  ...
  private createMilitaryVesselsLayer(
    ...
    return new ScatterplotLayer({
      ...
      getFillColor: (d) => {
        if (d.usniSource)
          return [255, 160, 60, 160] as [number, number, number, number]; // Orange, lower alpha for USNI-only
      ...
      getLineColor: (d) => {
        if (d.usniSource)
          return [255, 180, 80, 200] as [number, number, number, number]; // Orange outline
  ...
  private createMilitaryVesselClustersLayer(
    clusters: MilitaryVesselCluster[],
  ): ScatterplotLayer {
    return new ScatterplotLayer({
  ...
  private createMilitaryFlightClustersLayer(
    clusters: MilitaryFlightCluster[],
  ): ScatterplotLayer {
    return new ScatterplotLayer({
  ...
  private createStockExchangesLayer(): ScatterplotLayer {
    return new ScatterplotLayer({
      ...
      getPosition: (d) => [d.lon, d.lat],
      getRadius: (d) =>
        d.tier === "mega" ? 18000 : d.tier === "major" ? 14000 : 11000,
      getFillColor: (d) => {
        if (d.tier === "mega")
          return [255, 215, 80, 220] as [number, number, number, number];
  ...
  private createFinancialCentersLayer(): ScatterplotLayer {
    return new ScatterplotLayer({
      ...
      getPosition: (d) => [d.lon, d.lat],
      getRadius: (d) =>
        d.type === "global" ? 17000 : d.type === "regional" ? 13000 : 10000,
      getFillColor: (d) => {
        if (d.type === "global") return COLORS.financialCenter;
        if (d.type === "regional")
          return [0, 190, 130, 185] as [number, number, number, number];
  ...
  private createCentralBanksLayer(): ScatterplotLayer {
    return new ScatterplotLayer({
      ...
      getPosition: (d) => [d.lon, d.lat],
      getRadius: (d) =>
        d.type === "major" ? 15000 : d.type === "supranational" ? 17000 : 12000,
      getFillColor: (d) => {
  ...
  private createCommodityHubsLayer(): ScatterplotLayer {
    return new ScatterplotLayer({
      ...
      getPosition: (d) => [d.lon, d.lat],
      getRadius: (d) =>
        d.type === "exchange" ? 14000 : d.type === "port" ? 12000 : 10000,
      getFillColor: (d) => {
        if (d.type === "exchange") return COLORS.commodityHub;
        if (d.type === "port")
          return [80, 170, 255, 190] as [number, number, number, number];
  ...
  private createGulfInvestmentsLayer(): ScatterplotLayer {
    return new ScatterplotLayer<GulfInvestment>({
      ...
      getFillColor: (d: GulfInvestment) =>
        d.investingCountry === "SA"
          ? COLORS.gulfInvestmentSA
          : COLORS.gulfInvestmentUAE,
      getLineColor: [255, 255, 255, 80] as [number, number, number, number],
      lineWidthMinPixels: 1,
      radiusMinPixels: 5,
      radiusMaxPixels: 28,
  ...
  private hasRecentRiot(
    ...
    const hasRecentClusterRiot = this.protestClusters.some(
      (c) =>
        c.hasRiot &&
        c.latestRiotEventTimeMs != null &&
        now - c.latestRiotEventTimeMs < windowMs,
    );
    ...
    const layers: ScatterplotLayer[] = [
      new ScatterplotLayer({
        ...
        getFillColor: (d) => {
          ...
          const a = Math.round(
            (THREAT_ALPHA[d.threatLevel] || 120) * alphaScale,
          );
  ...
  private createPositiveEventsLayers(): Layer[] {
    ...
    if (significantEvents.length > 0) {
      const pulse =
        1.0 +
        0.4 * (0.5 + 0.5 * Math.sin((this.pulseTime || Date.now()) / 800));
      layers.push(
        new ScatterplotLayer({
  ...
  private createKindnessLayers(): Layer[] {
    ...
    const pulse =
      1.0 + 0.4 * (0.5 + 0.5 * Math.sin((this.pulseTime || Date.now()) / 800));
    layers.push(
      new ScatterplotLayer<KindnessPoint>({
        id: "kindness-pulse",
        data: this.kindnessPoints,
  ...
  private createHappinessChoroplethLayer(): GeoJsonLayer | null {
    ...
    return new GeoJsonLayer({
      ...
      getFillColor: (feature: { properties?: Record<string, unknown> }) => {
        const code = feature.properties?.["ISO3166-1-Alpha-2"] as
          | string
          | undefined;
  ...
  private static readonly CII_LEVEL_COLORS: Record<
    string,
    [number, number, number, number]
  > = {
    low: [40, 180, 60, 130],
  ...
  private createRenewableInstallationsLayer(): ScatterplotLayer {
    ...
    return new ScatterplotLayer({
      ...
      getFillColor: (d: RenewableInstallation) =>
        typeColors[d.type] ??
        ([200, 200, 200, 200] as [number, number, number, number]),
      stroked: true,
      getLineColor: (d: RenewableInstallation) =>
        typeLineColors[d.type] ??
  ...
  private getTooltip(info: PickingInfo): { html: string } | null {
    ...
    switch (layerId) {
      ...
      case "bases-cluster-layer":
        return {
          html: `<div class="deckgl-tooltip"><strong>${obj.count} bases</strong></div>`,
        };
      case "nuclear-layer":
        return {
      ...
      case "processing-plants-layer": {
        ...
        const mineralLabel =
          obj.mineral ??
          (Array.isArray(obj.materials) ? obj.materials.join(", ") : "");
        return {
      ...
      case "commodity-ports-layer": {
        const commoditiesStr = Array.isArray(obj.commodities)
          ? obj.commodities.join(", ")
          : "";
      ...
      case "cable-advisories-layer": {
        const cableName =
          UNDERSEA_CABLES.find((c) => c.id === obj.cableId)?.name ||
          obj.cableId;
        return {
      ...
      case "outages-layer":
        return {
          html: `<div class="deckgl-tooltip"><strong>${text(obj.asn || t("components.deckgl.tooltip.internetOutage"))}</strong><br/>${text(obj.country)}</div>`,
        };
      ...
      case "kindness-layer":
        return {
          html: `<div class="deckgl-tooltip"><strong>${text(obj.name)}</strong></div>`,
        };
      case "happiness-choropleth-layer": {
        const hcName = obj.properties?.name ?? "Unknown";
        const hcCode = obj.properties?.["ISO3166-1-Alpha-2"];
        const hcScore = hcCode
          ? this.happinessScores.get(hcCode as string)
          : undefined;
      ...
      case "cii-choropleth-layer": {
        ...
        const ciiEntry = ciiCode
          ? this.ciiScoresMap.get(ciiCode as string)
          : undefined;
        if (!ciiEntry)
          return {
      ...
      case "webcam-layer": {
        ...
        return {
          html: `<div class="deckgl-tooltip"><strong>${text(label)}</strong></div>`,
        };
      }
      ...
      case "ontario-roads-layer": {
        const severity = obj.severity || "UNKNOWN";
        const severityColor =
          severity === "CRITICAL"
            ? "#dc2626"
            : severity === "MAJOR"
        ...
        return {
          html: `<div class="deckgl-tooltip">
            <strong style="color: ${severityColor}">${text(obj.roadName)}</strong><br>
            ${text(obj.description || obj.type || "Incident")}<br>
            <span style="opacity: 0.7">${severity} \u00B7 ${text(obj.county || "Ontario")}</span>
          </div>`,
        };
      }
      default:
  ...
  private handleClick(info: PickingInfo): void {
    const isChoropleth = info.layer?.id
      ? DeckGLMap.CHOROPLETH_LAYER_IDS.has(info.layer.id)
      : false;
    ...
    if (layerId === "protest-clusters-layer") {
      ...
      if (
        ...
        try {
          ...
          console.warn(
            "[DeckGLMap] stale protest cluster",
            cluster._clusterId,
            e,
          );
          return;
    ...
    if (layerId === "tech-hq-clusters-layer") {
      ...
      if (
        ...
        try {
          const leaves = this.techHQSC.getLeaves(
            cluster._clusterId,
            DeckGLMap.MAX_CLUSTER_LEAVES,
          );
          cluster.items = leaves
          ...
          console.warn(
            "[DeckGLMap] stale techHQ cluster",
            cluster._clusterId,
            e,
          );
          return;
    ...
    if (layerId === "tech-event-clusters-layer") {
      ...
      if (
        ...
        try {
          ...
          console.warn(
            "[DeckGLMap] stale techEvent cluster",
            cluster._clusterId,
            e,
          );
          return;
    ...
    if (layerId === "datacenter-clusters-layer") {
      ...
      if (
        ...
        try {
          ...
          console.warn(
            "[DeckGLMap] stale datacenter cluster",
            cluster._clusterId,
            e,
          );
          return;
  ...
  private async showWebcamClickPopup(
    webcam: WebcamEntry,
    x: number,
    y: number,
  ): Promise<void> {
  ...
  private createLayerToggles(): void {
      ...
      input.addEventListener("change", () => {
        ...
        if (layer) {
          ...
          if (layer === "ciiChoropleth") {
            const ciiLeg = this.container.querySelector(
              "#ciiChoroplethLegend",
            ) as HTMLElement | null;
            if (ciiLeg)
              ciiLeg.style.display = (input as HTMLInputElement).checked
                ? "block"
                : "none";
          }
    ...
    const searchEl = toggles.querySelector(
      ".layer-search",
    ) as HTMLElement | null;
    ...
    collapseBtn?.addEventListener("click", () => {
      ...
      if (searchEl)
        searchEl.style.display = toggleList?.classList.contains("collapsed")
          ? "none"
          : "";
      if (collapseBtn)
  ...
  private createLegend(): void {
    ...
    const legendItems =
      SITE_VARIANT === "tech"
        ? [
            {
              shape: shapes.circle(
                isLight ? "rgb(22, 163, 74)" : "rgb(0, 255, 150)",
              ),
            ...
            {
              shape: shapes.circle(
                isLight ? "rgb(180, 120, 0)" : "rgb(255, 200, 0)",
              ),
        ...
        : SITE_VARIANT === "finance"
          ...
          : SITE_VARIANT === "happy"
            ...
            : [
                ...
                {
                  shape: shapes.circle(
                    isLight ? "rgb(180, 120, 0)" : "rgb(255, 255, 0)",
                  ),
                ...
                },
                {
                  shape: shapes.hexagon(
                    isLight ? "rgb(180, 120, 0)" : "rgb(255, 220, 0)",
                  ),
    ...
    ciiLegend.style.display = this.state.layers.ciiChoropleth
      ? "block"
      : "none";
    ciiLegend.innerHTML = `
  ...
  private updateZoomHints(): void {
    const toggleList = this.container.querySelector(
    ...
    for (const [key, enabled] of Object.entries(this.state.layers)) {
      ...
      if (!toggle) continue;
      const zoomHidden =
        !!enabled && !this.isLayerVisible(key as keyof MapLayers);
  ...
  public setLayers(layers: MapLayers): void {
    ...
    else if (!this.state.layers.weatherRadar && prevRadar)
  ...
  public setOntarioRoadsIncidents(incidents: OntarioRoadIncident[]): void {
    this.ontarioRoadsIncidents = incidents;
    this.render();
  }
  ...
  private hasAircraftViewportChanged(): boolean {
    ...
    return (
      Math.abs(center.lat - prevLat) > threshold ||
      Math.abs(center.lng - prevLng) > threshold
    );
  }
  ...
  public setCIIScores(
    scores: Array<{ code: string; score: number; level: string }>,
  ): void {
    this.ciiScoresMap = new Map(
      scores.map((s) => [s.code, { score: s.score, level: s.level }]),
    );
  ...
  private enforceLayerLimit(): void {
    ...
    const activeCount = Array.from(
      ...
      .filter(
        (i) =>
          (i.closest(".layer-toggle") as HTMLElement)?.style.display !== "none",
      )
      .filter((i) => i.checked).length;
    const increasing = activeCount > this.lastActiveLayerCount;
    this.lastActiveLayerCount = activeCount;
    if (
      activeCount >= WARN_THRESHOLD &&
      increasing &&
      !this.layerWarningShown
    ) {
  ...
  public triggerBaseClick(id: string): void {
    const base =
      this.serverBases.find((b) => b.id === id) ||
      MILITARY_BASES.find((b) => b.id === id);
    if (base) {
  ...
  private resolveCountryFromCoordinate(
    ...
    try {
      ...
      const properties = (features?.[0]?.properties ?? {}) as Record<
        string,
        unknown
      >;
      const code =
    ...
    getCountriesGeoJson()
      .then((geojson) => {
        ...
        this.updateCountryLayerPaint(
          isLightMapTheme(paintMapTheme) ? "light" : "dark",
        );
  ...
  private loadTorontoNeighbourhoods(): void {
    ...
    fetch("/data/toronto-neighbourhood-risk.geojson")
      ...
      .catch((err) =>
        console.warn("[DeckGLMap] Failed to load Toronto neighbourhoods:", err),
      );
  }
  ...
  private getHighlightRestOpacity(): { fill: number; border: number } {
    const theme = isLightMapTheme(getMapTheme(getMapProvider()))
      ? "light"
      : "dark";
  ...
  private switchToFallbackStyle(mapTheme: string): void {
    ...
    this.usedFallbackStyle = true;
    const fallback = isLightMapTheme(mapTheme)
      ? FALLBACK_LIGHT_STYLE
      : FALLBACK_DARK_STYLE;
    console.warn(
      `[DeckGLMap] Basemap tiles failed, falling back to OpenFreeMap: ${fallback}`,
    );
    if (this.countryPulseRaf) {
      cancelAnimationFrame(this.countryPulseRaf);
      this.countryPulseRaf = null;
