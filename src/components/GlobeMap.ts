...
export class GlobeMap {
  ...
  private webcamMarkerMode: string =
    localStorage.getItem("wm-webcam-marker-mode") || "icon";
  ...
  private async initGlobe(): Promise<void> {
    ...
    this.controlsEndHandler = () => {
      ...
      this.imageryFetchTimer = setTimeout(
    ...
    this.unsubscribeVisualPreset = subscribeGlobeVisualPresetChange(
      (preset) => {
    ...
    if (canvas) {
      ...
      canvas.addEventListener("webglcontextlost", (e) => {
        ...
        this.webglLost = true;
        console.warn(
          "[GlobeMap] WebGL context lost — will restore when browser recovers",
        );
      });
    ...
    (globe as any)
      ...
      .pathPointAlt((p: number[], _idx: number, path: object) =>
        (path as GlobePath)?.pathType === "orbit" && p.length > 2
          ? (p[2] ?? 0) / 6371
          : 0,
      )
      ...
      .pathStroke((d: GlobePath) => {
        ...
        if (d.pathType === "stormTrack" || d.pathType === "stormHistory")
          return 1.2;
        return 0.6;
      })
    ...
    (globe as any)
      .polygonGeoJsonGeometry((d: GlobePolygon) => ({
        type: "Polygon",
        coordinates: d.coords,
      }))
      .polygonCapColor((d: GlobePolygon) => {
        if (d._kind === "cii")
          return GlobeMap.CII_GLOBE_COLORS[d.level!] ?? "rgba(0,0,0,0)";
        if (d._kind === "conflict")
          return (
            GlobeMap.CONFLICT_CAP[d.intensity!] ?? GlobeMap.CONFLICT_CAP.low
          );
      ...
      .polygonSideColor((d: GlobePolygon) => {
        ...
        if (d._kind === "conflict")
          return (
            GlobeMap.CONFLICT_SIDE[d.intensity!] ?? GlobeMap.CONFLICT_SIDE.low
          );
        if (d._kind === "imageryFootprint") return "rgba(0,0,0,0)";
      ...
      .polygonStrokeColor((d: GlobePolygon) => {
        ...
        if (d._kind === "conflict")
          return (
            GlobeMap.CONFLICT_STROKE[d.intensity!] ??
            GlobeMap.CONFLICT_STROKE.low
          );
      ...
      .polygonAltitude((d: GlobePolygon) => {
        ...
        if (d._kind === "conflict")
          return (
            GlobeMap.CONFLICT_ALT[d.intensity!] ?? GlobeMap.CONFLICT_ALT.low
          );
        return 0.005;
      })
    ...
    getCountriesGeoJson()
      ...
      .catch((err) => {
        if (import.meta.env.DEV)
          console.warn("[GlobeMap] Failed to load countries GeoJSON", err);
      });
  }
  ...
  private buildMarkerElement(d: GlobeMarker): HTMLElement {
    ...
    if (d._kind === "conflict") {
      ...
      const cc =
        GlobeMap.CLUSTER_ACTIVITY_COLORS[d.activityType ?? "unknown"] ??
        "#6688aa";
      ...
      const mc =
        d.magnitude >= 6 ? "#ff2020" : d.magnitude >= 4 ? "#ff8800" : "#ffcc00";
      ...
      const emoji =
        this.webcamMarkerMode === "emoji" ? style.emoji : "\u{1F4F7}";
      el.innerHTML = GlobeMap.wrapHit(
  ...
  private showMarkerTooltip(d: GlobeMarker, anchor: HTMLElement): void {
    ...
    if (d._kind === "conflict") {
      html =
        ...
        (d.eventType
          ? `<br><span style="opacity:.7;">${esc(d.eventType)}</span>`
          : "") +
        (d.fatalities
          ? `<br><span style="opacity:.5;">Casualties: ${d.fatalities}</span>`
          : "");
    } else if (d._kind === "hotspot") {
      const sc =
        ["", "#88ff44", "#ffdd00", "#ffaa00", "#ff6600", "#ff2020"][
          d.escalationScore
        ] ?? "#ffaa00";
      html =
      ...
      const compass =
        dirs[Math.round(((((d.heading ?? 0) % 360) + 360) % 360) / 22.5) % 16];
      html =
      ...
      html =
        ...
        (actLabel
          ...
          : "") +
        (d.region
          ? `<br><span style="opacity:.6;font-size:10px;">${esc(d.region)}</span>`
          : "");
    } else if (d._kind === "weather") {
      const wc =
        d.severity === "Extreme"
      ...
      html =
        ...
        (d.deaths
          ? `<br><span style="opacity:.5;">Deaths: ${d.deaths}</span>`
          : "");
    } else if (d._kind === "displacement") {
      html =
      ...
      html =
        ...
        `<br><span style="opacity:.7;">${esc(d.country)}</span>` +
        (d.daysUntil >= 0
          ? `<br><span style="opacity:.5;">In ${d.daysUntil} days</span>`
          : "");
    } else if (d._kind === "conflictZone") {
      const ic =
        d.intensity === "high"
      ...
      const mc =
        d.magnitude >= 6 ? "#ff3030" : d.magnitude >= 4 ? "#ff8800" : "#ffcc00";
      html =
      ...
      html =
        ...
        (d.impact
          ...
          : "") +
        (d.repairEta
          ? `<br><span style="opacity:.5;">ETA: ${esc(d.repairEta)}</span>`
          : "");
    } else if (d._kind === "repairShip") {
      const sc = d.status === "on-station" ? "#44ff88" : "#44aaff";
      html =
        ...
        (d.eta
          ? `<br><span style="opacity:.5;">ETA: ${esc(d.eta)}</span>`
          : "");
    } else if (d._kind === "aisDisruption") {
      const sc =
        d.severity === "high"
          ...
          : d.severity === "elevated"
            ...
            : "#44aaff";
      const typeLabel =
        d.type === "gap_spike" ? "Gap Spike" : "Chokepoint Congestion";
      html =
      ...
      const operatorName =
        SAT_OPERATOR_NAME[d.country] ||
        getCountryNameByCode(d.country) ||
        d.country;
    ...
    if (d._kind === "webcam") {
      ...
      link.rel = "noopener";
      link.style.cssText =
        "display:block;color:#00d4ff;font-size:11px;text-decoration:none;";
      ...
      import("@/services/webcams").then(({ fetchWebcamImage }) => {
        fetchWebcamImage(d.webcamId).then((img) => {
          ...
          if (img.thumbnailUrl) {
            ...
            imgEl.style.cssText =
              "width:200px;border-radius:4px;margin-bottom:4px;";
            imgEl.loading = "lazy";
    ...
    const top = Math.max(
      4,
      Math.min(ar.top - cr.top - 8, cr.height - el.offsetHeight - 4),
    );
    ...
    if (d._kind === "webcam-cluster") {
      ...
      import("@/services/webcams").then(
        ({ fetchWebcams, getClusterCellSize }) => {
          ...
          fetchWebcams(10, {
            ...
            for (const webcam of webcams) {
              ...
              nameSpan.textContent =
                webcam.title || webcam.category || "Webcam";
              item.appendChild(nameSpan);

              if (webcam.country) {
                const countrySpan = document.createElement("span");
                countrySpan.style.cssText =
                  "float:right;opacity:0.4;font-size:10px;margin-left:6px;";
  ...
  private createLayerToggles(): void {
    ...
    el.querySelectorAll(".layer-toggle input").forEach((input) => {
      input.addEventListener("change", () => {
        ...
        if (layer) {
          ...
          if (layer === "webcams") {
            const modeRow = el.querySelector(
              ".webcam-mode-row",
            ) as HTMLElement | null;
    ...
    if (webcamToggleEl) {
      ...
      modeRow.style.cssText =
        "display:none;padding:2px 6px 4px 24px;font-size:10px;color:#aaa;";
    ...
    collapseBtn?.addEventListener("click", () => {
      ...
      if (collapseBtn)
        (collapseBtn as HTMLElement).innerHTML = collapsed
          ? "&#9654;"
          : "&#9660;";
    });
  ...
  private flushMarkers(): void {
    if (!this.globe || !this.initialized || this.destroyed || this.webglLost)
      return;
  ...
  private flushPaths(): void {
    ...
    const paths =
      showCables && showPipelines
        ? this.globePaths
        : this.globePaths.filter((p) =>
            p.pathType === "cable" ? showCables : showPipelines,
          );
      ...
      for (const z of CONFLICT_ZONES) {
        ...
        if (isoCodes && this.countriesGeoData) {
          for (const feat of this.countriesGeoData.features) {
            const code = feat.properties?.["ISO3166-1-Alpha-2"] as
              | string
              | undefined;
    ...
    switch (layer) {
      ...
      case "irradiators":
        if (!this.irradiatorSiteMarkers.length) {
          this.irradiatorSiteMarkers = (
            GAMMA_IRRADIATORS as GammaIrradiator[]
          ).map((g) => ({
      ...
      case "economic":
        if (!this.economicMarkers.length) {
          this.economicMarkers = (ECONOMIC_CENTERS as EconomicCenter[]).map(
            (c) => ({
      ...
      case "waterways":
        if (!this.waterwayMarkers.length) {
          this.waterwayMarkers = (
            STRATEGIC_WATERWAYS as StrategicWaterway[]
          ).map((w) => ({
      ...
      case "minerals":
        if (!this.mineralMarkers.length) {
          this.mineralMarkers = (CRITICAL_MINERALS as CriticalMineralProject[])
            .filter(
              (m) => m.status === "producing" || m.status === "development",
            )
            .map((m) => ({
  ...
  > = new Map([
    [
      "ciiChoropleth",
      { markers: false, arcs: false, paths: false, polygons: true },
    ],
    [
      "tradeRoutes",
      { markers: false, arcs: true, paths: false, polygons: false },
    ],
    [
      "pipelines",
      { markers: false, arcs: false, paths: true, polygons: false },
    ],
  ...
  private enforceLayerLimit(): void {
    ...
    const activeCount = Array.from(
      this.layerTogglesEl.querySelectorAll<HTMLInputElement>(
        ".layer-toggle input",
      ),
  ...
  public setFlightDelays(delays: AirportDelayAlert[]): void {
    ...
    this.notamRingMarkers = (delays ?? [])
      .filter(
        (d) => d.lat != null && d.lon != null && d.delayType === "closure",
      )
      .map((d) => ({
  ...
  private static latLngAltToVec3(
    lat: number,
    lng: number,
    alt: number,
    vec3Ctor: any,
  ...
  private async rebuildSatBeams(positions: SatellitePosition[]): Promise<void> {
    ...
    for (const s of positions) {
      const groundCenter = GlobeMap.latLngAltToVec3(
        s.lat,
        s.lng,
        0,
        THREE.Vector3,
    ...
    if (allRayPositions.length > 0) {
      ...
      rayGeo.setAttribute(
        "position",
        new THREE.Float32BufferAttribute(allRayPositions, 3),
      );
      rayGeo.setAttribute(
        "color",
        new THREE.Float32BufferAttribute(allRayColors, 3),
      );
    ...
    if (allConePositions.length > 0) {
      ...
      coneGeo.setAttribute(
        "position",
        new THREE.Float32BufferAttribute(allConePositions, 3),
      );
      coneGeo.setAttribute(
        "color",
        new THREE.Float32BufferAttribute(allConeColors, 3),
      );
