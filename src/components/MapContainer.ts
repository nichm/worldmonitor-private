...
export class MapContainer {
  ...
  public setTorontoFireIncidents(incidents: TorontoFireIncident[]): void {
    if (this.useGlobe) {
      this.globeMap?.setTorontoFireIncidents(incidents);
      return;
    }
    if (this.useDeckGL) {
      this.deckGLMap?.setTorontoFireIncidents(incidents);
    }
  ...
  public setOntarioRoadsIncidents(incidents: OntarioRoadIncident[]): void {
    if (this.useGlobe) {
      this.globeMap?.setOntarioRoadsIncidents(incidents);
      return;
    }
    if (this.useDeckGL) {
      this.deckGLMap?.setOntarioRoadsIncidents(incidents);
    }
    // SVG map does not support Ontario roads incidents
  }

  public setTorontoDineSafe(closures: DineSafeClosure[]): void {
    if (this.useGlobe) {
      this.globeMap?.setTorontoDineSafe(closures);
      return;
    }
    if (this.useDeckGL) {
      this.deckGLMap?.setTorontoDineSafe(closures);
    }
