...
const MUNICIPALITY_COORDS: Record<string, { lat: number; lon: number }> = {
  ...
  'Burlington': { lat: 43.3250, lon: -79.8000 },
 ...
 */
export function getRecentSpills(spills: SpillRecord[], limit: number = 5): SpillRecord[] {
  return spills
    .sort((a, b) => new Date(b.spillDate).getTime() - new Date(a.spillDate).getTime())
    .slice(0, limit);
}
 ...
 */
export function getSpillsByMunicipality(spills: SpillRecord[]): Record<string, number> {
