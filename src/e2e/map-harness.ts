...
const getLayerDataCount = (layerId: string): number => {
  return (
    getDeckLayerSnapshot().find((layer) => layer.id === layerId)?.dataCount ?? 0
  );
};
...
const getFirstProtestTitle = (): string | null => {
  const layers = internals.buildLayers?.() ?? [];
  const protestLayer = layers.find(
    (layer) => layer.id === "protest-clusters-layer",
  );
...
const [irradiatorLon, irradiatorLat] = firstLatLon(
  GAMMA_IRRADIATORS,
  [12.0, 50.0],
);
...
const [datacenterLon, datacenterLat] = firstLatLon(
  AI_DATA_CENTERS,
  [-121.9, 37.3],
);
...
const [acceleratorLon, acceleratorLat] = firstLatLon(
  ACCELERATORS,
  [-122.41, 37.77],
);
const [techHQLon, techHQLat] = firstLatLon(TECH_HQS, [-122.0, 37.3]);
const [cloudRegionLon, cloudRegionLat] = firstLatLon(
  CLOUD_REGIONS,
  [-122.3, 37.6],
);
...
const [financialCenterLon, financialCenterLat] = firstLatLon(
  FINANCIAL_CENTERS,
  [-74.0, 40.7],
);
const [centralBankLon, centralBankLat] = firstLatLon(
  CENTRAL_BANKS,
  [-77.0, 38.9],
);
const [commodityHubLon, commodityHubLat] = firstLatLon(
  COMMODITY_HUBS,
  [-87.6, 41.8],
);
...
const buildPulseProtests = (
  scenario: PulseProtestScenario,
): SocialUnrestEvent[] => {
...
const prepareVisualScenario = (scenarioId: string): boolean => {
  ...
  map.setNewsLocations(
    scenario.includeNewsLocation ? SEEDED_NEWS_LOCATIONS : [],
  );
