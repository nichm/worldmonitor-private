...
import {
  DEFAULT_PANELS as TORONTO_PANELS,
  DEFAULT_MAP_LAYERS as TORONTO_MAP_LAYERS,
  MOBILE_DEFAULT_MAP_LAYERS as TORONTO_MOBILE_MAP_LAYERS,
} from "./variants/toronto";
...
const COMMODITY_PANELS: Record<string, PanelConfig> = {
  ...
  economic: { name: "Macro Stress", enabled: true, priority: 1 },
  "gulf-economies": {
    name: "Gulf & OPEC Economies",
    enabled: true,
    priority: 1,
  },
  "gcc-investments": {
    name: "GCC Resource Investments",
    enabled: true,
    priority: 2,
  },
...
export const DEFAULT_PANELS: Record<string, PanelConfig> = Object.fromEntries(
  (VARIANT_DEFAULTS[SITE_VARIANT] ?? VARIANT_DEFAULTS["full"] ?? []).map(
    (key) => [key, getEffectivePanelConfig(key, SITE_VARIANT)],
  ),
);
